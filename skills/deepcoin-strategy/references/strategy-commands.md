...
```bash
deepcoin-cli strategy backtest --symbol <INST_ID> --from-ts <iso-time> --to-ts <iso-time> --dsl @strategy.json [--json]
deepcoin-cli strategy dsl-trigger-order --symbol <INST_ID> --trade-mode <cross|isolated> --mrg-position <merge|split> --dsl @strategy.json
```
